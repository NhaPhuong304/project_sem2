...
public class StaffRepository extends BaseRepository {
	...
	public Staff findByAccountId(int accountId) {
		String sql = "SELECT s.StaffID, s.FullName, s.Email, s.AccountID, "
				+ "a.Username, a.PhotoUrl, a.IsActive, a.[Role] " + "FROM Staff s "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "WHERE s.AccountID = ?";
		try {
	...
	public Staff findById(int staffId) {
		String sql = "SELECT s.StaffID, s.FullName, s.Email, s.AccountID, "
				+ "a.Username, a.PhotoUrl, a.IsActive, a.[Role] " + "FROM Staff s "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "WHERE s.StaffID = ?";
		try {
			return executeQuery(sql, rs -> {
				if (rs.next())
					return mapRow(rs);
	...
	public void updateStaff(int staffId, String username, String fullName, String email) {
		String sql = "UPDATE Staff SET FullName = ?, Email = ? WHERE StaffID = ?";
		try {
			executeUpdate(sql, fullName, email, staffId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in updateStaff: " + e.getMessage(), e);
		}
	}
	...
	public void updateTeacherFullName(int staffId, String fullName) {
		...
		try {
			executeUpdate(sql, fullName, staffId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in updateTeacherFullName: " + e.getMessage(), e);
		}
	}
	public void updateStaffFullName(int staffId, String fullName) {
	    ...
	    try {
	        ...
	        throw new RuntimeException("DB error in updateStaffFullName: " + e.getMessage(), e);
	    }
	}

	public void toggleStaffStatus(int staffId) {
		String sql = "UPDATE Account " + "SET IsActive = CASE WHEN IsActive = 1 THEN 0 ELSE 1 END "
				+ "WHERE AccountID = (SELECT AccountID FROM Staff WHERE StaffID = ?)";
		try {
			executeUpdate(sql, staffId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in toggleStaffStatus: " + e.getMessage(), e);
		}
	}
	...
	public List<Staff> findAll() {
		String sql = "SELECT s.StaffID, s.FullName, s.Email, s.AccountID, "
				+ "a.Username, a.PhotoUrl, a.IsActive, a.[Role] " + "FROM Staff s "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "ORDER BY s.FullName";
		try {
			return executeQuery(sql, rs -> {
	...
	public List<Staff> findByRole(UserRole role) {
		String sql = "SELECT s.StaffID, s.FullName, s.Email, s.AccountID, "
				+ "a.Username, a.PhotoUrl, a.IsActive, a.[Role] " + "FROM Staff s "
				+ "INNER JOIN Account a ON a.AccountID = s.AccountID " + "WHERE a.[Role] = ? " + "ORDER BY s.FullName";
		try {
			return executeQuery(sql, rs -> {
				List<Staff> list = new ArrayList<>();
				while (rs.next())
	...
	public Staff findByEmail(String email) {
		String sql = "SELECT s.StaffID, s.FullName, s.Email, s.AccountID, "
				+ "a.Username, a.PhotoUrl, a.IsActive, a.[Role] " + "FROM Staff s "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "WHERE s.Email = ?";
		try {
			return executeQuery(sql, rs -> {
				if (rs.next())
					return mapRow(rs);
