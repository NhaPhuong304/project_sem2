...
public class ClassRepository extends BaseRepository {
	...
	public List<SchoolClass> findAll() {
		String sql = "SELECT c.ClassID, c.ClassName, c.AcademicYear, c.ManagerID, st.FullName AS ManagerName, c.CreatedAt, "
				+ "COUNT(s.StudentID) AS StudentCount " + "FROM Class c LEFT JOIN Student s ON s.ClassID = c.ClassID "
				+ "LEFT JOIN Staff st ON st.StaffID = c.ManagerID " + "WHERE c.ClassName <> ? "
	...
	public List<SchoolClass> findByManagerId(int managerId) {
		String sql = "SELECT c.ClassID, c.ClassName, c.AcademicYear, c.ManagerID, st.FullName AS ManagerName, c.CreatedAt, "
				+ "COUNT(s.StudentID) AS StudentCount " + "FROM Class c LEFT JOIN Student s ON s.ClassID = c.ClassID "
				+ "LEFT JOIN Staff st ON st.StaffID = c.ManagerID " + "WHERE c.ClassName <> ? AND c.ManagerID = ? "
				+ "GROUP BY c.ClassID, c.ClassName, c.AcademicYear, c.ManagerID, st.FullName, c.CreatedAt "
				+ "ORDER BY c.CreatedAt DESC";
		try {
			return executeQuery(sql, rs -> {
				...
				while (rs.next())
					list.add(mapRow(rs));
				return list;
			}, UNASSIGNED_CLASS_NAME, managerId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in findByManagerId classes: " + e.getMessage(), e);
		}
	}

	public List<SchoolClass> findByAdvisorId(int staffId) {
		String sql = "SELECT c.ClassID, c.ClassName, c.AcademicYear, c.ManagerID, st.FullName AS ManagerName, c.CreatedAt, "
				+ "(SELECT COUNT(*) FROM Student s WHERE s.ClassID = c.ClassID) AS StudentCount " + "FROM Class c "
				+ "LEFT JOIN Staff st ON st.StaffID = c.ManagerID " + "WHERE c.ClassName <> ? " + "AND EXISTS ("
				+ "    SELECT 1 FROM Project p " + "    INNER JOIN ProjectGroup pg ON pg.GroupID = p.GroupID "
				+ "    WHERE pg.ClassID = c.ClassID AND p.AdvisorID = ?" + ") " + "ORDER BY c.CreatedAt DESC";
		try {
			return executeQuery(sql, rs -> {
				List<SchoolClass> list = new ArrayList<>();
				while (rs.next())
	...
	public List<SchoolClass> findByStaffId(int staffId) {
		String sql = "SELECT ClassID, ClassName, AcademicYear FROM Class WHERE ManagerID = ?";
		try {
			return executeQuery(sql, rs -> {
				List<SchoolClass> list = new ArrayList<>();
				while (rs.next()) {
			...
			throw new RuntimeException("DB error in findByStaffId: " + e.getMessage(), e);
		}
	}

	public int create(String className, String academicYear, Integer managerId) {
		String sql = "INSERT INTO Class (ClassName, AcademicYear, ManagerID) VALUES (?, ?, ?)";
		try {
			return executeUpdateGetKey(sql, className, academicYear, managerId);
		} catch (SQLException e) {
	...
	public SchoolClass findById(int classId) {
		String sql = "SELECT c.ClassID, c.ClassName, c.AcademicYear, c.ManagerID, st.FullName AS ManagerName, c.CreatedAt, "
				+ "COUNT(s.StudentID) AS StudentCount " + "FROM Class c LEFT JOIN Student s ON s.ClassID = c.ClassID "
				+ "LEFT JOIN Staff st ON st.StaffID = c.ManagerID " + "WHERE c.ClassID = ? "
				+ "GROUP BY c.ClassID, c.ClassName, c.AcademicYear, c.ManagerID, st.FullName, c.CreatedAt";
	...
	public int ensureUnassignedClass() {
		...
		return create(UNASSIGNED_CLASS_NAME, "N/A", null);
	}
