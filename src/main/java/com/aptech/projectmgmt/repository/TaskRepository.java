...
public class TaskRepository extends BaseRepository {
	...
	public int create(Task task) {
		...
		try {
			return executeUpdateGetKey(sql, task.getGroupId(), task.getTitle(), task.getDescription(),
					task.getStatus().getValue(), task.getAssignedTo(), task.getReviewedBy(), task.getCreatedBy(),
	...
	public void insertRevision(int taskId, String note, int reviewedBy, LocalDateTime createdAt) {
		...
		try {
			executeUpdate(sql, taskId, note, reviewedBy, createdAt != null ? Timestamp.valueOf(createdAt) : null);
		} catch (SQLException e) {
	...
	public List<TaskStatusHistory> getStatusHistory(int taskId) {
		String sql = "SELECT tsh.HistoryID, tsh.TaskID, tsh.FromStatus, tsh.ToStatus, tsh.ChangedBy, tsh.ChangedAt, "
				+ "COALESCE(s.FullName, st.FullName, a.Username) AS ChangerName " + "FROM TaskStatusHistory tsh "
		...
		try {
			return executeQuery(sql, rs -> {
				...
				while (rs.next()) {
					...
					if (!rs.wasNull())
						h.setFromStatus(TaskStatus.fromValue(from));
					...
					if (ts != null)
						h.setChangedAt(ts.toLocalDateTime());
	...
	public List<TaskAbandonLog> getAbandonLogs(int taskId) {
		String sql = "SELECT tal.LogID, tal.TaskID, tal.StudentID, tal.AbandonedAt, s.FullName AS StudentName "
				+ "FROM TaskAbandonLog tal " + "INNER JOIN Student s ON s.StudentID = tal.StudentID "
		...
		try {
			return executeQuery(sql, rs -> {
				...
				while (rs.next()) {
					...
					if (ts != null)
						log.setAbandonedAt(ts.toLocalDateTime());
	...
	public int resetOverdueTasks(LocalDateTime currentTime) {
		String selectSql = "SELECT TaskID, GroupID, AssignedTo " + "FROM Task "
	...
	private TaskViewModel mapViewModelRow(ResultSet rs) throws SQLException {
		...
		if (es != null)
			t.setEstimatedStartDate(es.toLocalDateTime());
		Timestamp ee = rs.getTimestamp("EstimatedEndDate");
		if (ee != null)
			t.setEstimatedEndDate(ee.toLocalDateTime());
		Timestamp as = rs.getTimestamp("ActualStartDate");
		if (as != null)
			t.setActualStartDate(as.toLocalDateTime());
		Timestamp ae = rs.getTimestamp("ActualEndDate");
		if (ae != null)
			t.setActualEndDate(ae.toLocalDateTime());
		...
		if (!rs.wasNull())
			t.setAssignedMemberStatus(MemberStatus.fromValue(assignedStatus));
		int reviewedById = rs.getInt("ReviewedByID");
		if (!rs.wasNull())
			t.setReviewedById(reviewedById);
		t.setReviewedByName(rs.getString("ReviewedByName"));
		int reviewedStatus = rs.getInt("ReviewedMemberStatus");
		if (!rs.wasNull())
			t.setReviewedMemberStatus(MemberStatus.fromValue(reviewedStatus));
	...
	public void assignTask(int taskId, int assignedStudentId, int changedByStudentId) {
		String sql = "UPDATE Task SET AssignedTo = ? WHERE TaskID = ?";
		try {
			executeUpdate(sql, assignedStudentId, taskId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in assignTask: " + e.getMessage(), e);
		}
	}

	public void reassignTask(int taskId, int assignedStudentId, LocalDateTime estimatedStartDate,
			LocalDateTime estimatedEndDate, int changedByStudentId) {
		String sql = "UPDATE Task SET AssignedTo = ?, EstimatedStartDate = ?, EstimatedEndDate = ? WHERE TaskID = ?";
		try {
			executeUpdate(sql, assignedStudentId, Timestamp.valueOf(estimatedStartDate),
					Timestamp.valueOf(estimatedEndDate), taskId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in reassignTask: " + e.getMessage(), e);
		}
	}

	public List<TaskViewModel> getTasksByStudentInGroup(int groupId, int studentId) {
		String sql = "SELECT p.ProjectID, p.Title AS ProjectTitle, p.Semester, c.ClassName, pg.GroupID, pg.GroupName, "
				+ "t.TaskID, t.Title, t.[Description] AS TaskDetail, t.EstimatedStartDate, t.EstimatedEndDate, "
				+ "t.ActualStartDate, t.ActualEndDate, t.[Status] AS Status, t.IsLate, "
				+ "assigned.FullName AS AssignedToName, assignedAccount.PhotoUrl AS AssignedToPhoto, "
				...
				+ "           GROUP BY TaskID, ReceiverID) reminder "
				+ "       ON reminder.TaskID = t.TaskID AND reminder.ReceiverID = t.AssignedTo "
				+ "WHERE pg.GroupID = ? " + "  AND (t.AssignedTo = ? OR t.ReviewedBy = ?) "
		...
		try {
			return executeQuery(sql, rs -> {
				...
				return list;
			}, NotificationUtil.REMINDER_PREFIX + "%", groupId, studentId, studentId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in getTasksByStudentInGroup: " + e.getMessage(), e);
		}
	}
