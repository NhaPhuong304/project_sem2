...
public class ProjectRepository extends BaseRepository {
	...
	private static final String BASE_SELECT = "SELECT p.ProjectID, p.Title AS ProjectName, p.[Description] AS Description, p.Semester, "
			...
			+ "st.FullName AS SupervisorName " + "FROM Project p "
	...
	public List<Project> findByClassId(int classId) {
		String sql = BASE_SELECT + "WHERE pg.ClassID = ? ORDER BY p.CreatedAt DESC, p.ProjectID DESC";
		try {
	...
	public List<Project> findByStaffManagedClasses(int staffId) {
		String sql = BASE_SELECT + "INNER JOIN Class c ON c.ClassID = pg.ClassID " + "WHERE c.ManagerID = ? "
		...
		try {
			...
			throw new RuntimeException("DB error in findByStaffManagedClasses: " + e.getMessage(), e);
		}
	}

	public List<Project> findByCreatedByStaffId(int staffId) {
		String sql = BASE_SELECT + "WHERE p.CreatedBy = ? ORDER BY p.CreatedAt DESC, p.ProjectID DESC";
		try {
			return executeQuery(sql, rs -> {
				List<Project> list = new ArrayList<>();
				while (rs.next()) {
			...
			throw new RuntimeException("DB error in findByCreatedByStaffId: " + e.getMessage(), e);
		}
	}
	...
	public List<Project> findByAdvisorId(int advisorId) {
		String sql = BASE_SELECT + "WHERE p.AdvisorID = ? ORDER BY p.CreatedAt DESC, p.ProjectID DESC";
		try {
			return executeQuery(sql, rs -> {
				List<Project> list = new ArrayList<>();
				while (rs.next())
	...
	public List<Project> findByStudentId(int studentId) {
		String sql = "SELECT p.ProjectID, p.Title AS ProjectName, p.[Description] AS Description, p.Semester, pg.ClassID, "
				...
				+ "st.FullName AS SupervisorName, gm.Role AS MyRole " + "FROM GroupMember gm "
				+ "INNER JOIN ProjectGroup pg ON pg.GroupID = gm.GroupID "
				+ "INNER JOIN Project p ON p.GroupID = pg.GroupID " + "LEFT JOIN Staff st ON st.StaffID = p.AdvisorID "
				+ "WHERE gm.StudentID = ? AND gm.Status = 1 " + "ORDER BY p.StartDate DESC";
		try {
			return executeQuery(sql, rs -> {
				List<Project> list = new ArrayList<>();
				while (rs.next()) {
					...
					if (!rs.wasNull())
						p.setMyRole(MemberRole.fromValue(myRoleVal));
					list.add(p);
				}
				return list;
			}, studentId);
	...
	public int create(Project project) {
		...
		try {
			return executeUpdateGetKey(sql, project.getGroupId(), project.getProjectName(), project.getDescription(),
					project.getSemester(), project.getStartDate() != null ? Date.valueOf(project.getStartDate()) : null,
	...
	public void update(Project project) {
		...
		try {
			executeUpdate(sql, project.getProjectName(), project.getDescription(), project.getSemester(),
					project.getSupervisorId(),
	...
	private Project mapRow(ResultSet rs) throws SQLException {
		...
		if (ed != null)
			p.setEndDate(ed.toLocalDate());
		Date rd = rs.getDate("ReportDate");
		if (rd != null)
			p.setReportDate(rd.toLocalDate());
