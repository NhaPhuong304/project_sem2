...
public class GroupRepository extends BaseRepository {
	...
	public List<ProjectGroup> findByProjectId(int projectId) {
		String sql = "SELECT pg.GroupID, pg.ClassID, pg.GroupName, p.ProjectID " + "FROM Project p "
				+ "INNER JOIN ProjectGroup pg ON pg.GroupID = p.GroupID " + "WHERE p.ProjectID = ? "
	...
	public void reactivateMember(int memberId) {
		...
		try {
			executeUpdate(sql, MemberStatus.ACTIVE.getValue(), memberId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in reactivateMember: " + e.getMessage(), e);
		}
	}

	public void updateMemberRole(int memberId, MemberRole role) {
		String sql = "UPDATE GroupMember SET Role = ? WHERE MemberID = ?";
		try {
			executeUpdate(sql, role.getValue(), memberId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in updateMemberRole: " + e.getMessage(), e);
		}
	}

	public List<GroupMember> findActiveMembersExcept(int groupId, int excludeMemberId) {
		String sql = "SELECT gm.MemberID, gm.GroupID, gm.StudentID, gm.Role, gm.Status, "
				...
				+ "FROM GroupMember gm " + "INNER JOIN Student s ON s.StudentID = gm.StudentID "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID "
				+ "WHERE gm.GroupID = ? AND gm.MemberID <> ? AND gm.Status = ? " + "ORDER BY s.FullName ASC";
		try {
			return executeQuery(sql, rs -> {
				...
				return list;
			}, groupId, excludeMemberId, MemberStatus.ACTIVE.getValue());
		} catch (SQLException e) {
			throw new RuntimeException("DB error in findActiveMembersExcept: " + e.getMessage(), e);
		}
	}

	public void transferLeader(int oldLeaderMemberId, int newLeaderMemberId) {
	...
	public void transferLeaderAndExclude(int oldLeaderMemberId, int newLeaderMemberId, int staffId, String reason) {
		String demoteOldLeaderSql = "UPDATE GroupMember SET Role = ? WHERE MemberID = ?";
		String promoteNewLeaderSql = "UPDATE GroupMember SET Role = ? WHERE MemberID = ?";
		String excludeOldLeaderSql = "UPDATE GroupMember SET Status = 2, ExcludedBy = ?, ExcludedAt = GETDATE(), ExcludedReason = ? WHERE MemberID = ?";

		try (java.sql.Connection conn = com.aptech.projectmgmt.config.DatabaseConfig.getConnection()) {
			conn.setAutoCommit(false);
			try (java.sql.PreparedStatement ps1 = conn.prepareStatement(demoteOldLeaderSql);
					java.sql.PreparedStatement ps2 = conn.prepareStatement(promoteNewLeaderSql);
					java.sql.PreparedStatement ps3 = conn.prepareStatement(excludeOldLeaderSql)) {
				ps1.setInt(1, MemberRole.MEMBER.getValue());
				ps1.setInt(2, oldLeaderMemberId);
				ps1.executeUpdate();
				...
				ps3.setInt(3, oldLeaderMemberId);
				ps3.executeUpdate();
			...
			throw new RuntimeException("DB error in transferLeaderAndExclude: " + e.getMessage(), e);
		}
	}

	public List<String> getMemberNamesByGroupId(int groupId) {
		String sql = "SELECT s.FullName " + "FROM GroupMember gm "
				+ "INNER JOIN Student s ON s.StudentID = gm.StudentID " + "WHERE gm.GroupID = ? AND gm.Status = ? "
				+ "ORDER BY s.FullName ASC";
		try {
			return executeQuery(sql, rs -> {
				List<String> memberNames = new ArrayList<>();
				while (rs.next()) {
					memberNames.add(rs.getString("FullName"));
				}
				return memberNames;
			}, groupId, MemberStatus.ACTIVE.getValue());
		} catch (SQLException e) {
			throw new RuntimeException("DB error in getMemberNamesByGroupId: " + e.getMessage(), e);
		}
	}

	public ProjectGroup findById(int groupId) {
		String sql = "SELECT pg.GroupID, pg.ClassID, pg.GroupName, p.ProjectID " + "FROM ProjectGroup pg "
				+ "LEFT JOIN Project p ON p.GroupID = pg.GroupID " + "WHERE pg.GroupID = ?";
		try {
			return executeQuery(sql, rs -> {
				if (rs.next()) {
	...
	public boolean existsStudentInProject(int projectId, int studentId) {
		String sql = "SELECT COUNT(*) AS Cnt " + "FROM GroupMember gm "
		...
		try {
			Integer count = executeQuery(sql, rs -> rs.next() ? rs.getInt("Cnt") : 0, projectId, studentId,
					MemberStatus.ACTIVE.getValue(), ProjectStatus.ACTIVE.getValue());
			return count != null && count > 0;
		} catch (SQLException e) {
	...
	public boolean hasActiveLeader(int groupId) {
		...
		try {
			Integer count = executeQuery(sql, rs -> rs.next() ? rs.getInt("Cnt") : 0, groupId,
					MemberRole.LEADER.getValue(), MemberStatus.ACTIVE.getValue());
			return count != null && count > 0;
		} catch (SQLException e) {
	...
	public List<GroupMember> findMembersByGroupId(int groupId) {
		String sql = "SELECT gm.MemberID, gm.GroupID, gm.StudentID, gm.Role, gm.Status, "
				...
				+ "s.StudentCode, s.FullName AS StudentFullName, a.PhotoUrl AS StudentPhotoUrl "
				+ "FROM GroupMember gm " + "INNER JOIN Student s ON s.StudentID = gm.StudentID "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "WHERE gm.GroupID = ? "
	...
	public List<Student> findAvailableStudentsForClass(int classId) {
		String sql = "SELECT s.StudentID, s.StudentCode, s.FullName, s.Email, s.ClassID, s.AccountID, a.PhotoUrl "
				+ "FROM Student s " + "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "WHERE s.ClassID = ? "
				+ "AND NOT EXISTS ( " + "    SELECT 1 " + "    FROM GroupMember gm "
				+ "    INNER JOIN ProjectGroup pg ON pg.GroupID = gm.GroupID "
				+ "    INNER JOIN Project p ON p.GroupID = pg.GroupID "
	...
	public GroupMember findMemberByStudentAndGroup(int studentId, int groupId) {
		String sql = "SELECT gm.MemberID, gm.GroupID, gm.StudentID, gm.Role, gm.Status, "
				...
				+ "s.StudentCode, s.FullName AS StudentFullName, a.PhotoUrl AS StudentPhotoUrl "
				+ "FROM GroupMember gm " + "INNER JOIN Student s ON s.StudentID = gm.StudentID "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "WHERE gm.StudentID = ? AND gm.GroupID = ?";
		try {
			return executeQuery(sql, rs -> {
				if (rs.next())
	...
	public GroupMember findMemberById(int memberId) {
		String sql = "SELECT gm.MemberID, gm.GroupID, gm.StudentID, gm.Role, gm.Status, "
				...
				+ "s.StudentCode, s.FullName AS StudentFullName, a.PhotoUrl AS StudentPhotoUrl "
				+ "FROM GroupMember gm " + "INNER JOIN Student s ON s.StudentID = gm.StudentID "
				+ "LEFT JOIN Account a ON a.AccountID = s.AccountID " + "WHERE gm.MemberID = ?";
		try {
			return executeQuery(sql, rs -> {
				if (rs.next()) {
					return mapMemberRow(rs);
	...
	private GroupMember mapMemberRow(ResultSet rs) throws SQLException {
		...
		if (!rs.wasNull())
			m.setExcludedBy(excludedBy);
		Timestamp excludedAt = rs.getTimestamp("ExcludedAt");
		if (excludedAt != null)
			m.setExcludedAt(excludedAt.toLocalDateTime());
