...
public class StudentRepository extends BaseRepository {
	...
	public List<Student> findWithoutAccount(int classId) {
		String sql = "SELECT s.StudentID, s.StudentCode, s.FullName, s.Email, s.ClassID, s.AccountID, a.PhotoUrl, a.IsActive, c.ClassName, s.CreatedByStaffId, st.FullName as CreatedByStaffName "
				+ "FROM Student s " + "LEFT JOIN Account a ON a.AccountID = s.AccountID "
				+ "LEFT JOIN Class c ON c.ClassID = s.ClassID "
				+ "LEFT JOIN Staff st ON st.StaffID = s.CreatedByStaffId "
	...
	public String getNextStudentCode() {
		String sql = "SELECT TOP 1 StudentCode " + "FROM Student " + "WHERE StudentCode LIKE 'ST%' "
	...
	public boolean isStudentWorkingOnProject(int studentId) {
		String sql = "SELECT COUNT(*) AS Total " + "FROM GroupMember gm "
				+ "INNER JOIN ProjectGroup pg ON pg.GroupID = gm.GroupID "
				+ "INNER JOIN Project p ON p.GroupID = pg.GroupID " + "WHERE gm.StudentID = ? " + "AND gm.Status = 1 "
				+ "AND p.Status = 1";
		try {
			return executeQuery(sql, rs -> {
				if (rs.next()) {
			...
			throw new RuntimeException("DB error in isStudentWorkingOnProject: " + e.getMessage(), e);
		}
	}

	public void updateStudentAccountStatus(int studentId, boolean isActive) {
		String sql = "UPDATE Account " + "SET IsActive = ? "
				+ "WHERE AccountID = (SELECT AccountID FROM Student WHERE StudentID = ?)";

		try {
			executeUpdate(sql, isActive, studentId);
		} catch (SQLException e) {
			throw new RuntimeException("DB error in updateStudentAccountStatus: " + e.getMessage(), e);
		}
	}
