...
public class LoginController {
    ...
    private void handleLogin() {
        ...
        task.setOnFailed(e -> {
            ...
            showError(ex != null ? ex.getMessage() : "Login failed.");
        });
    ...
    private void handleForgotPassword() {
        String username = usernameField.getText().trim();
        if (username.isEmpty()) {
            showError("Please enter the username first.");
            return;
        }
        Task<Account> task = new Task<>() {
            @Override
        ...
        task.setOnFailed(e -> {
            Throwable ex = task.getException();
            showError(ex != null ? ex.getMessage() : "System error.");
        });
        new Thread(task).start();
    }
    ...
    private void navigateToOtp(int accountId, OtpPurpose purpose) {
        try {
            ...
            showError("Failed to open the OTP screen: " + ex.getMessage());
        }
    }
    ...
    private void navigateToAdminDashboard() {
        try {
            ...
            showError("Failed to switch screens: " + ex.getMessage());
        }
    }
