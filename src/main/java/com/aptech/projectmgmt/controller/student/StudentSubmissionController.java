...
public class StudentSubmissionController {
	...
	private void setupTargetTable() {
		requestTitleColumn.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getRequestTitle()));
		deadlineColumn.setCellValueFactory(
				data -> new SimpleStringProperty(formatDateTime(data.getValue().getRequestDeadline())));
		targetStatusColumn
				.setCellValueFactory(data -> new SimpleStringProperty(formatStatus(data.getValue().getStatus())));
	...
	private void loadTargets() {
		...
		task.setOnSucceeded(e -> Platform.runLater(() -> {
			...
			if (targets.isEmpty()) {
				clearDetail(
						"You do not have any project submission requests yet. Only group leaders can see requests here.");
				setLoading(false, "");
				return;
			}
	...
	private void loadTargetDetail(SubmissionTarget target) {
		...
		Task<DetailData> task = new Task<>() {
			...
			protected DetailData call() {
				return new DetailData(submissionService.getRequirements(target.getRequestId()),
	...
	private void chooseAndUpload(SubmissionRequirement requirement) {
		...
		Task<Void> task = new Task<>() {
			@Override
			protected Void call() {
				submissionService.saveSubmittedFile(selectedTarget.getTargetId(), requirement, selectedFile, studentId);
				return null;
			}
		};
		task.setOnSucceeded(e -> Platform.runLater(() -> {
	...
	private void handleCompleteSubmission() {
		...
		if (!AlertUtil.showConfirm(
				"Confirm project submission completion? After completion, you will not be able to submit again from this screen.")) {
			return;
		}
