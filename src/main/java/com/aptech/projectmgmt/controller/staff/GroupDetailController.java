...
public class GroupDetailController {
		...
		return new TableCell<>() {
			...
			protected void updateItem(Void item, boolean empty) {
				...
				if (member.getStatus() == MemberStatus.EXCLUDED) {
					...
					primaryBtn.setStyle(
							"-fx-text-fill: #3b82f6; -fx-font-size: 14px; -fx-background-color: #eff6ff; -fx-background-radius: 6; -fx-padding: 4 8; -fx-cursor: hand;");
	...
	private void handleChangeLeader(GroupMember newLeader) {
		...
		if (!AlertUtil.showConfirm("Are you sure you want to assign " + newLeader.getStudentFullName() + " as the new leader?")) {
			return;
		}
	...
	private void handleExclude(GroupMember member) {
		...
		if (member.getRole() == MemberRole.LEADER && member.getStatus() == MemberStatus.ACTIVE) {
			...
			if (candidates.isEmpty()) {
				AlertUtil.showError(
						"Cannot revoke the leader's participation because the group has no active member available to replace them.");
				return;
			}
			...
			Task<Void> task = new Task<>() {
				@Override
				protected Void call() {
					groupService.excludeLeaderAndTransfer(member.getMemberId(), newLeader.getMemberId(),
							currentStaff.getStaffId(), reason);
					return null;
				}
			};
	...
	private void handleHardDelete(GroupMember member) {
		...
		if (member.getRole() == MemberRole.LEADER && member.getStatus() == MemberStatus.ACTIVE) {
			...
			if (candidates.isEmpty()) {
				AlertUtil.showError("Cannot delete the leader because there is no member available to replace them.");
				return;
			}
			...
			if (!AlertUtil.showConfirm(
					"Are you sure you want to transfer leadership and delete " + member.getStudentFullName() + "?")) {
				return;
			}
			...
			Task<Void> task = new Task<>() {
				@Override
				protected Void call() {
					groupService.deleteLeaderAndTransfer(member.getMemberId(), newLeader.getMemberId());
					return null;
				}
			};

			task.setOnSucceeded(e -> Platform.runLater(() -> {
				AlertUtil.showSuccess("Leader transferred and member deleted");
				loadMembers();
			}));
		...
		// Xóa member thường
		if (!AlertUtil
				.showConfirm("Are you sure you want to delete " + member.getStudentFullName() + " from the group?")) {
			return;
		}
	...
	private Optional<GroupMember> showChooseNewLeaderDialog(List<GroupMember> candidates) {
		...
		dialog.setHeaderText(
				"This member is the group leader. Please choose a new leader before revoking their participation.");
	...
	private Optional<String> showReasonDialog(String studentName) {
		try {
			...
			controller.initData("Reason for revoking participation",
					"Enter the reason for revoking participation of " + studentName, "");
