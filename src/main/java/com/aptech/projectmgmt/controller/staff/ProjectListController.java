...
public class ProjectListController {
	...
	private void loadClasses() {
		Task<List<SchoolClass>> task = new Task<>() {
			...
			protected List<SchoolClass> call() {
				...
				if (staffId != null) {
					return classService.getClassesByStaff(staffId);
				}
	...
	private void handleEditProject(Project project) {
	    ...
	    loadStaffTask.setOnSucceeded(e -> Platform.runLater(() -> {
	        try {
	            ...
	            controller.setSupervisors(loadStaffTask.getValue());
	            controller.setData(project);

	            Dialog<ButtonType> dialog = new Dialog<>();
	            dialog.setTitle("Sua project");
	            ...
	            Project updatedProject = controller.buildUpdatedProject(project);
	...
	private TableCell<Project, Void> createActionCell() {
		return new TableCell<>() {
			...
			protected void updateItem(Void item, boolean empty) {
				...
				editBtn.setDisable(readOnlyMode);
				setGraphic(box);
			}
		};
	}
	...
	private void loadProjects() {
		Task<List<Project>> task = new Task<>() {
			...
			protected List<Project> call() {
				...
				if (staffId != null) {
					return projectService.getProjectsByCreatedStaff(staffId);
				}
	...
	private void refreshSemesterOptions() {
		...
		allProjects.stream()
				...
				.map(Project::getSemester).filter(semester -> semester != null && !semester.isBlank()).distinct()
